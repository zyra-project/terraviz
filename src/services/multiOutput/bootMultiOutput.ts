...
import { MultiOutputManager, createTauriHost, type MultiOutputHost } from './manager'
...
export function startMultiOutput(
  ...
  const ready = (options.createHost ?? createTauriHost)()
    .then(host => {
      // stop() ran while the host was being built. Constructing a manager
      // now would leave one nothing owns.
      if (stopped) return null
      manager = new MultiOutputManager(host)

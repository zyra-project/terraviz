 ...
 *     knowledge.
 *
 * The pre-search `[RELEVANT DATASETS]` injection (Phase 1c) was
 * removed in 1d/F when the cutover relied on the LLM to call
 * `search_datasets` itself, then re-introduced in 1d/AC after live
 * testing showed mid-tier models (llama-4-scout, sometimes 3.1-70b)
 * confabulate id-shaped strings rather than copying tool-result
 * IDs verbatim. The injection's source is now the Vectorize-backed
 * `search_datasets` rather than the legacy in-memory keyword scan,
 * so the architectural goals of the cutover (single search source
 * of truth, federation-ready) are preserved while the docent's
 * chip-render reliability matches the pre-cutover regime.
 *
...
1. **[RELEVANT DATASETS] in the user message** — when present, this block contains pre-searched results with \`id\`, \`title\`, \`categories\`, and a description snippet, sourced from the same Vectorize semantic search the \`search_datasets\` tool would call. **Use these FIRST.** They are the highest-confidence source for the current query and you do not need to call any discovery tool when they are present — every recommendation should pull its \`id\` from this block.
2. **\`search_datasets\` tool** — semantic vector search over the node catalog. Use this when the [RELEVANT DATASETS] block is missing or doesn't have a suitable match for a follow-up query. Result shape: \`{ datasets: [{ id, title, abstract_snippet, categories, peer_id, score }] }\`.
3. **\`list_featured_datasets\` tool** — small operator-curated list (no query). Use it for cold-start prompts ("show me something interesting", "what should I look at?", "I don't know where to start") when the user has not expressed a topic. Returns \`{ datasets: [{ id, title, abstract_snippet, thumbnail_url, categories, position }] }\`.
4. **\`search_catalog\` tool** — legacy in-memory keyword scan over the catalog. **Use this ONLY as a last-ditch fallback** when both [RELEVANT DATASETS] and \`search_datasets\` come up empty (typically a self-hosting deploy without Vectorize provisioned). Returns up to 10 results with \`id\`, \`title\`, \`categories\`, \`description\`. **Never invent dataset titles or IDs from training-time knowledge.**
...
WORKFLOW:
1. Check whether the user message contains a [RELEVANT DATASETS] block. If so, pick the best 1–3 matches from it; you do not need to call any discovery tool. Copy each row's \`id\` field verbatim into the marker.
2. Otherwise — if [RELEVANT DATASETS] is absent OR the user is asking a follow-up about a different topic than the block covers — call \`search_datasets\` silently. If \`search_datasets\` returns an empty \`datasets\` array, fall back to \`search_catalog\` (or \`list_featured_datasets\` for cold-start prompts) before giving up — never invent dataset titles or IDs.
3. Recommend the chosen 1–3 matches in prose, referring to each by its exact \`title\`.
4. **MANDATORY**: Every dataset title you mention MUST be immediately followed by a \`<<LOAD:FULL_DATASET_ID>>\` marker on its own line, using the exact \`id\` field from whichever source provided it ([RELEVANT DATASETS] or a tool result). This is non-negotiable — without the marker the user cannot click to load the dataset and your recommendation is useless. Mentioning a title in prose without the marker is a bug, not an option.

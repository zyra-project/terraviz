...
| `src/output/linkWatchdog.ts` | Has the control window gone quiet, and what should the output do about it (`docs/MULTI_MONITOR_PLAN.md` §3 "Failure recovery", case 3)? The manager broadcasts at least once a second — `STATE_TICK_MS`, the heartbeat that sends a full snapshot even when nothing changed — so silence is measurable and means something. Past `IPC_STALE_MS` the link is **stale**: the output keeps rendering its last known state, because a frozen frame of the right dataset beats a black sphere in front of an audience, and starts pinging so a manager that is merely busy can notice and resync. Past `IPC_ORPHAN_MS` it is **orphaned** and stops pinging — nobody is listening, and a window that pings forever burns IPC on a dead channel for the rest of an installation's uptime. Pure, and split from `outputLink` for `playbackSettle`'s reason: every decision here is about *elapsed time*, which is the thing most miserable to test through a real clock when the thresholds are five and sixty seconds. Three rules are easy to get backwards. **The clock starts at connect, not at the first message** — an output nobody ever broadcasts to is the case most worth detecting (it is what a manager that never opened the link looks like), and a watchdog armed by the first arrival sits `live` forever in exactly that case. **Orphaned is not terminal** — the plan's recovery is the manager's boot scan finding the window after a control-window relaunch and sending a fresh snapshot, so any message at any time returns it to `live`. **Both thresholds measure from the last message**, not from each other, or the orphan lands at 65 s for a reason nobody could read off the constants. `LINK_PING_INTERVAL_MS` stays here rather than in `protocol.ts`, whose timings are the ones both ends must agree on: only the output sends these and the manager answers whatever arrives, so putting it there would imply a coupling that does not exist |
| `src/output/main.ts` | Output-window entry point — resolves the canvas, builds the scene, attaches the link, drives the rAF loop. Thin on purpose, the shape `orbitMain.ts` has over `orbitCharacter/`, so the testable logic lives in `outputScene.ts` / `outputLink.ts` / `datasetMirror.ts` / `outputSync.ts` and needs no page to run. Two decisions live only here. **The link is optional; the page is not** — `output.html` is also the static fixture rungs 2-4 render in an ordinary browser, so the link attaches on desktop only and its failure costs the link rather than the render loop; a correct idle Earth beats a black window. **Playback is steered per frame, not per message** — a diff says where the primary *was* when it was sent and both clocks keep running between diffs, so steering only on arrival sawtooths; `outputSync` is idempotent inside the hard-seek threshold (it trims the rate rather than seeking), which is what makes per-frame calling correct. The composite is rebuilt from the **mirror**, never from the link: the link's `dataset` is what the control window says and the mirror's is what this window has actually decoded, so compositing the former would put an incoming dataset's bbox and palette over the outgoing dataset's pixels for the length of a load Rung 11 adds the HUD's composition: the overlay is mounted **unconditionally and hidden** (a hidden `refresh` reads nothing, so it costs one no-op timer callback and buys an instant toggle), the fps meter is ticked on **drawn** frames rather than rAF callbacks (the question is whether this output is painting, and for static content the honest answer is the 1 Hz floor) and sampled from the loop rather than from the HUD's reader (so the window stays ~500 ms while drawing at 1 Hz, and the reader stays pure — `sample()` resets the window, so a hidden HUD would otherwise open on an average of however long it was hidden), and the renderer string is queried once behind an `undefined`-means-unasked marker so a driver that *refuses* is not re-asked twice a second forever. `link.renderConfig()` is read once after subscribing, not merely defensively: the manager answers `output_ready` with a config that can land while `connectOutputLink` is still awaiting its own emit. The HUD's **fps** and **raf** are zeroed when the loop has not reported for three sample windows, and that is the third instance of one rule rather than a new idea: every frame number is written by the render loop and read by the overlay's own ~2 Hz timer, which keeps running when the loop does not — so an occluded output, a stalled compositor or a sleeping display would repaint the last healthy reading forever over a frozen sphere. It is the one direction neither **gpu** nor **link** covers: the context is fine and the control window is talking. `drawMs` is deliberately not zeroed, because it answers *capacity* and "the last frame cost 4 ms" stays true after the frames stop. Rung 13's case 3 rides the same steer callback: `link.checkHealth()` is called per frame rather than on a timer of its own — the loop already runs every frame and floors its *draw* at 1 Hz, either of which is ample against a five-second threshold, and the watchdog rations its own pings so the call costs an integer compare. A health transition deliberately does **not** flag the scene dirty: the picture does not change when the link does, and a stale link that forced redraws would spend GPU announcing that nothing is arriving. `readLinkHealth` is hoisted above the overlay and defaults to `live`, because the HUD is mounted unconditionally while the link attaches on desktop only inside a `try` that may fail — and a fixture page with no control window has not been abandoned by one. A diff flags the scene dirty only when it `changesPicture`, because the alternative — anything that changed is worth a frame — meant the playhead keys, which arrive at the control window's frame rate, drove this loop past the 30 fps cap for exactly the content the cap exists to bound. Case 5 adds one condition and one subscription. The loop **declines to draw while the context is lost**, and the reason is the bookkeeping rather than the draw: `render()` is already harmless on a dead context (Three returns immediately), but ticking the fps meter, clearing `dirty` and advancing `lastFrame` around it makes the HUD report a healthy 30 fps over a black projector — the exact shape of invisible failure this rung exists to remove. Skipping instead means fps falls to 0 on the next sample and `dirty` survives the outage rather than being cleared by a frame that reached no pixels. The subscription flags dirty on **`restored` only**, so the first good frame does not wait out the 1 Hz static floor; the `lost` edge needs nothing, because the skip is what preserves the flag. | Rung 14b's calibration is two lines of substitution here and nothing else: the pattern's whole lifecycle — when it is rebuilt, when a failed build is retried, why it is not discarded when the toggle goes off — lives in `calibrationPattern.ts`'s `createCalibrationCache`, where it is testable. The pattern goes in **instead of** the mirrored dataset, never over it (a graticule on top of data leaves neither legible, and what is being checked is geometry), and the mirror is left untouched underneath so its decoder keeps running and keeps being steered — turning calibration off puts the dataset back *in step* rather than reloading it. `readCalibration` is hoisted above the overlay and defaults to `false` for `readLinkHealth`'s reason, and the HUD needs it because its `dataset` field answers *what is on the glass*: calibration is the one state where the mirror still holds a dataset that is not being shown, so without it the HUD would name a dataset over a test pattern — the precise thing its "mirror, not link" rule exists to prevent, arriving from the other side.
| `src/output/outputScene.ts` | The output window's scene and render loop — the 2:1 equirectangular framebuffer (widths snapped **down** to a supported rung, because overshooting spends GPU memory on hardware that reported less), the fullscreen quad carrying `equirectRtt`'s pass — now **composited** with `layerStack`'s unrolled overlay slots, so a mirrored dataset actually reaches the sphere. The material is rebuilt only when the slot *count* changes, because GLSL ES 1.00 has no dynamic sampler indexing and the shader text is therefore a function of the count; everything else about a layer (texture, bbox, palette) is a uniform write, so an operator nudging a palette costs an upload rather than a compile. The **same** uniforms object survives a rebuild — fresh ones would snap every output back to a centred camera whenever a layer appeared. A slot's map texture is reused when the element is identical, since rebuilding a `VideoTexture` restarts the upload path for a change the decoder never saw, while the 1 KB palette is rebuilt every time; slots that go away are disposed **and their uniform entries nulled** — disposing is only half of it, since `uniforms` is long-lived and keyed by slot name and a Three texture holds its `image`, so a left-behind reference pins one decoded video element per removed slot for the life of the window even though the rebuilt shader no longer samples it. `OutputLayerInput` carries `kind` rather than sniffing `instanceof HTMLVideoElement`, which would make the video path the one path no test could reach. And the draw decision, via `contentKindFor`: 30 fps for a video **that is actually advancing**, 1 Hz for anything static, and immediately whenever something changed. `shouldRenderFrame` is a **carried-deadline** test, not `sinceLastFrame >= interval`, and it took two goes. A plain `>=` **aliases** against a display whose refresh equals the cap: an output on a 30 Hz monitor is offered callbacks 33.33 ms apart against a 33.33 ms video interval, so whether one cleared the gate came down to jitter in the last decimal — and every callback that missed waited a whole further one, turning a 33 ms frame into a 67 ms frame. The loop settled at ~22 fps while being offered exactly 30, identically on RGB and data-encoded video, which is what three passes of content-specific hypotheses were chasing. The first fix asked the right question — *is this callback closer to the target than the next one will be* — of the wrong quantity: `sinceLastFrame`, measured from the previous **draw**, which throws the phase away every frame and so leaves the rate a knife edge wherever `interval / offered` lands near a half-integer. A sweep says that is where most displays are: 75 Hz gave 25 fps, 48 and 50 gave 24 and 25, 144 and 165 gave 28.8 and 28.7, and 33 / 35 / 40 / 100 drew on every callback and ran *over* the cap. Only exact multiples of 30 came out right — the set the test sampled, which is why it passed. So the deadline is **carried**: `advanceFrameDeadline` moves it on one interval from where it was *due*, never from when the frame was drawn, so an early or late draw costs that frame's phase rather than the rate (45 Hz alternates one and two callbacks per draw and averages 30), a tie in the last decimal moves one frame across a boundary and nothing else, and a display slower than the cap degrades to its own rate with the deadline clamped so it cannot accumulate debt for the life of an installation. The one non-obvious clause is the interval *shrinking* — static → video — where the standing deadline is a second out and would stall the first video frame; it cannot legitimately sit more than one interval ahead, so that is the test. `sinceLastCallbackMs` is required rather than optional because a default of zero *is* the original bug. Read off the element rather than latched from the dataset — an output whose operator paused, or whose date fell outside the dataset's span, holds one frame, and pacing that at 30 fps is 30× the GPU for an identical picture on hardware that may drive sixteen of these; off the *element* rather than the sync outcome, because a dataset with no time axis is left looping by design and pacing that off the outcome would judder it. The 1 Hz floor is not an optimisation to remove — an output that never redraws cannot tell a dropped upload or a lost context from a correct frame. Lazy-loads Three behind the same `loadThree` seam `globeThumbnail` uses, and `photorealEarth` behind a matching `createEarth` one — consumed as a **texture provider** (its progressive 2K→4K→8K base diffuse, swapped in on `onBaseDiffuseChange` and flagged dirty so the upgrade does not wait out the 1 Hz floor) rather than as a mesh to render, because the equirect pass *is* the renderer; every mesh-only effect is switched off at construction. The sampler is bound to `baseEarthTexture` from the first frame and **never** to `null`: black on an output is indistinguishable from a dropped upload or a lost context, which is the one failure the 1 Hz floor exists to surface, and it shipped that way once. Rung 12c wires the decoration the plan's §"What the equirect path does to the Earth decoration" says crosses. **The sun is derived, never borrowed:** `uSunDir` is `latLonToDirection(getSunPosition(now))` — the same function `cameraOffsetForCamera` uses — recomputed on every **drawn** frame, unthrottled because that is pure arithmetic. It first *copied* `earth.sunDir`, on the reasoning that sharing `getSunPosition` stops the two globes disagreeing; that was true of the source and false of the **frame**, since `photorealEarth.sunDirectionFromLatLng` negates Z for the globe *mesh*, so the borrowed vector arrived longitude-mirrored and lit the opposite hemisphere. Sharing the frame is the property that matters, and deriving it here makes that true by construction. **Clouds are loaded here, not through the Earth handle** (`loadCloudImage`, defaulting to `getCloudTextureUrl()`): this composite needs the raw asset because that module bakes luminance to alpha at a gamma tuned for a lit shell seen from outside, and splicing it in washed the day side grey and clamped the night side to black. A failed load costs the clouds and nothing else. The two decoration samplers are bound to `baseEarthTexture` from the first frame and gated by `uHasNightLights` / `uHasCloud`, the same never-bind-null rule the sphere sampler follows, and each arrival flags the scene dirty so it does not wait out the 1 Hz floor. Deliberately **no** dirty flag for the sun: it moves ~0.004° a second, and flagging that would hold a static output at the render rate forever to animate something invisible `setFramebufferWidth` resizes the **drawing buffer, never the window** — `setSize(w, h, false)` leaves the CSS size alone, which is what makes a rung below the monitor's pixel count scale up rather than shrink into a corner — and no-ops on an unchanged snapped size so re-picking the current rung does not reallocate 128 MiB. `rendererName()` reads `WEBGL_debug_renderer_info` for the HUD and returns `null` rather than throwing when a driver refuses. **Every texture this shader samples is put into display space** by `useDisplaySpace`, and that is a correction rather than a preference: `photorealEarth` tags its diffuse and night lights `SRGBColorSpace`, so Three uploads them with an sRGB internal format and the *sampler* decodes to linear — right for that module's lighting pipeline, wrong here, where one quad renders straight to the default framebuffer with no `colorspace_fragment` chunk to re-encode, and where every `layerStack` constant is copied from `earthTileLayer`, which grades in sRGB **display** space off the MapLibre framebuffer. Measured cost of the mismatch: lit land `rgb(181, 150, 103)` reached the framebuffer as `rgb(112, 68, 30)`, and dark vegetation `rgb(27, 47, 19)` as `rgb(2, 5, 17)` — **byte-identical to ocean**, so forest and sea were one colour. It predates the colour grade; rung 12c's decoration inherited it too. The decode is *removed* rather than compensated for, which makes every copied constant correct by construction, and brings the base and lights into line with the cloud texture, built here by `new Texture(img)` and therefore always at Three's default `NoColorSpace`. Retagging another module's texture is safe because `createPhotorealEarth` builds them per call and this window owns its instance; it is applied again in each change callback because a 2K→4K→8K tier upgrade hands over a texture this module has never seen — retagging only at construction would look fixed for the first seconds of every launch and be wrong after Rung 13's case 5 adds `gpuState()` / `onGpuStateChange` over the canvas's own `webglcontextlost` / `webglcontextrestored`, and the interesting part is how little of the plan's prescription is left to write. Three's `WebGLRenderer` registers its own listeners **inside the constructor** (before the context exists, so it catches a creation-time loss), calls the `preventDefault()` the plan asks for, sets the flag that makes `render()` return immediately, and on restore runs `initGLContext()` — which builds a fresh `WebGLProperties` and with it fresh `textures`, `geometries`, `programCache` and `bindingStates`, so every GL handle is re-uploaded lazily on the next draw. This scene holds **no raw GL handles at all** — no `WebGLRenderTarget`, no `createTexture`, and the "2:1 framebuffer" is the renderer's own drawing buffer via `setSize(w, h, false)` — which is exactly why it is not `earthTileLayer`, whose closure-held `datasetTex` is what #403 found could never come back. So what this module adds is **observation, not repair**: three states (`live` / `lost` / `restored`) that report what the two DOM events said and deliberately claim nothing about the glass, since it cannot see it. No second `preventDefault()`, because a redundant one would imply this works without Three's — and if that ever went away, `render()` would throw on a dead context long before the missing call mattered. `dispose()` unhooks **before** `renderer.forceContextLoss()`, and that order is the correctness: `forceContextLoss` fires the same event a driver crash fires, so unhooking after it makes every ordinary teardown report a GPU failure and closing four outputs at the end of a show read as four crashes. It shipped once with a `disposing` flag instead, which the unhook already made unreachable — caught by mutating the code and watching the test pass anyway, then fixed by making the *fake renderer* dispatch the event so the ordering is what the test pins |
| `src/output/equirectRtt.ts` | The equirectangular render-to-texture pass for a multi-monitor output (`docs/MULTI_MONITOR_PLAN.md` §3) — for each pixel of a 2:1 framebuffer, the direction it represents is ray-marched from a configurable camera position against the unit sphere and the sphere texture sampled at the hit. One pass; the cubemap-and-convert route is rejected outright and must not be built. A centred camera is the identity; moving it off-centre magnifies the hemisphere it moved toward and compresses the antipode, and **that warp is the zoom** — the primary v1 mode, matching SOS behaviour. Because the camera is strictly inside the sphere every ray hits, so the shader has no miss branch and the far side shrinks rather than clipping. Everything but the GLSL strings is a pure TS mirror of the shader, so the projection is unit-testable with no GL context (same split as `datasetProbe.ts`); no Three.js import — commit 3 builds the material around it. Layer compositing (`layerStack.ts`) is deliberately absent. Rung 14 adds `uRotationOffsetRad`, the per-installation longitude turn, and **where** it applies is the correctness: on the longitude the split fold produced, not on the fold's input. `foldSplitU` is periodic in U with period ½, so rotating before it would make a 180° offset a **no-op** on exactly the installations most likely to run split mode. The plan's snippet also ends `mod(lon + offset, TWO_PI)`, which is dead arithmetic — the only consumers are `cos` and `sin` and the offset is bounded to one turn — so it is left out rather than transcribed, keeping the GLSL and the TS mirror one line each |
...
| `src/services/datasetLoader.ts` | Loads a dataset onto the globe (HLS or image); manages info panel. `pickDirectFile` is exported because it is where a wrong answer lives. The two readiness rules this path depends on — nudging a passive engine's pipeline, and declining to rewind into a hole at the head of `buffered` — were written here and now live in [`src/utils/mediaReadiness.ts`](src/utils/mediaReadiness.ts), because this module imports `playbackController`, which needs the second of them. Everything that fails between `new HLSService()` and the return is the caller's to survive and this module's to clean up: the service is **destroyed before the throw escapes**, since it is handed over only on success, so a leaked one is unreachable — and since the nudge, a leaked one is also *playing*, holding a decoder and a network fetch for the life of the window after a load the operator was told had failed |
...
| `src/analytics/transport.ts` | `fetch()` + `sendBeacon()` transport with response classification (ok/retry/permanent). The endpoint is **relative on web and absolute on desktop**, and that split is the whole point: on web the Pages Function answering `/api/ingest` is part of the deploy that served the page, so a fork reports to itself with no configuration — while a Tauri webview is served from `tauri://localhost/`, where the same relative path either fails to parse as a URL inside the Rust HTTP plugin (caught, classified retryable, retried forever with backoff and persisted to localStorage between launches) or returns the bundled `index.html`. The docstring here *claimed* the endpoint was absolute on Tauri and pointed at a `createFetchTransport({ endpoint })` override no caller ever passed, so it described a requirement nothing implemented; the origin now comes from `getApiOrigin()`, the same resolution `/api/v1/*` already uses, rather than a second build-time variable meaning the same thing. Fixing the URL is not the same as proving a row lands: `ingest.ts` 403s an unrecognised `Origin` and a 403 is dropped without retry, and whether the Tauri plugin — which issues from Rust, not the webview — forwards one has not been checked against a live deploy. `isTauri()` is a function rather than a module-load constant so a test can flip `window.__TAURI__` between cases |
...
| `src/config/endpoints.ts` | Externally-hosted endpoint configuration (catalog / proxy / NOAA / NASA base URLs), plus `getApiOrigin()` — **this node's own** origin, the one a desktop build needs because a webview served from `tauri://localhost/` has no Pages Functions backend behind a relative `/api/` path. It lives here rather than in `catalogSource.ts`, where it was written, because `analytics/transport.ts` needs the same origin for `/api/ingest` and `catalogSource` imports the analytics barrel — reaching across would close a cycle (transport → catalogSource → analytics → emitter → transport). This module imports nothing at all, which is what makes it the safe home; `catalogSource` re-exports it so its four existing callers are unchanged. One resolution rather than two, so a node's catalog and its telemetry cannot disagree about where "here" is |
...
| `src/ui/domUtils.ts` | Small DOM helpers shared across UI modules — the escaping pair, and `announcePolite`, the one write into the app-wide `#a11y-announcer`. Shared rather than copied because the clear-then-set-next-frame dance is the part easy to leave out and impossible to notice: assigning a live region the text it already holds is not a change, so the second identical announcement is dropped — which is exactly the shape of one worth making twice |
...
| `src/utils/mediaReadiness.ts` | When can a `<video>` actually show something, and where — `waitForDecodableFrame` and `playableStart`, written in `datasetLoader.ts` and moved out when a second and third caller arrived. Both answer a question the obvious code gets wrong, and both were first reached on **WebKitGTK**, the engine a Linux SOS installation runs. The wait **nudges** with a muted `play()` rather than only listening for `canplay`, because that engine does not preroll a media pipeline until something plays it — so a caller that waits for `canplay` before playing waits for itself (`readyState 1`, 88 s buffered, no error, a 20 s timeout blaming the user's connection). `playableStart` is the same rule from the other side: rewinding to a literal `0` parks a decodable element on a hole (`buffered [6.0, 94.1]` on a 94.1 s asset), and `readyState` is defined at the *playback position*, so it shows nothing and reports nothing. It covers **every** rewind, not only the load — an earlier note claimed the restart was a native `loop` beyond the app's reach, and nothing in the app sets `video.loop`; the restarts are `playbackController`'s own auto-loop and rewind button, so a load-only fix held for exactly one pass of the asset. Here rather than in `datasetLoader.ts` because that module imports `playbackController`, which needs `playableStart` (a cycle), and because the multi-monitor output needs the wait in a bundle kept clear of the SPA loader graph. Imports nothing at all, which is what makes it safe for both. `pauseWhenReady` is the output's: its playhead is steered by `outputSync`, so it wants the preroll without the playback, and it pauses on the resolve path only — a rejection means the nudge never took |
...
> its own commit), calibration (14), and the rest of rung 13 — the
> monitor-unplug poll, the single HLS rebuild, and case 5's 30 s
> no-restore timeout with the `gpu-loss-timeout` removal it triggers
> (that one auto-closes a window on a projector, and should not be
> written before a forced `WEBGL_lose_context` on hardware has said
> whether a restore actually arrives).
> Rung 13's other slices *have* landed. The manager notices a window
> that went away and tells a crash from a hand-close (13a), and it
> reports `output_added` / `output_removed` / `output_failure` as Tier A
> telemetry (13b) — so each remaining failure-recovery case adds one
> `emit()` rather than re-opening the analytics reviewer checklist, a
> claim **case 6 then cashed**: its reporters reuse `output_removed`'s
> `monitor-gone` and `crash` and `output_failure`'s `ipc-silence` with
> no schema change at all. **Case 3** followed: an output notices its
> control window going quiet (`linkWatchdog`), and the panel badges it
> and announces the transition. **Case 6** after that: a manager whose
> page reloaded now *adopts* the `output-*` windows that outlived it,
> instead of leaving them rendering a frame nobody is driving. **Case
> 5's detection and reporting** landed last, and turned out to be a
> fraction of what the plan sized: Three's `WebGLRenderer` already
> registers both listeners in its constructor, already calls the
> `preventDefault()` the plan prescribes, already makes `render()` a
> no-op while lost, and already rebuilds its GL state on restore via
> `initGLContext()` — and the output's scene holds no raw GL handles
> for that to miss, which is exactly what `earthTileLayer` is not. So
> what landed is observation, not repair: `gpuState()` on the scene,
> `output_gpu_lost` / `output_gpu_recovered` on the wire, a **Display
> lost** badge, and the `gpu-loss` Tier A kind rung 13b had already
> declared. It also found a live bug — the render loop was ticking the
> fps meter and clearing `dirty` around a `render()` that drew nothing,
> so the HUD reported a healthy 30 fps over a black projector. Case 6
> also corrects the plan — it is a control-window **reload** that
> leaves orphans, never a crash, because every window lives in the one
...
| `src-tauri/capabilities/output.json` | Narrow capability scoped to the `output-*` window glob — IPC listen/emit, the window getters and setters an output drives **on itself**, and HTTPS fetch with localhost explicitly denied. `allow-destroy` is deliberately **absent**, and arriving there took two tries. Tauri's `onCloseRequested` completes the close from JS by calling `destroy()` on the window itself when the handler does not `preventDefault()`, and that call is ACL-checked against the *output* — so granting `close` alone made every output unclosable (the manager's Remove, the operator's Alt+F4, everything), because the denial happens inside Tauri's own listener callback where nothing in this repo can see it, while `handle.close()` resolves normally on the manager side and `discard()` goes on to drop the record. Row gone, window alive, task manager the only way out; found on hardware. Granting `destroy` fixed that and gave away more than it fixed: neither `close` nor `destroy` is scoped to the *calling* window, so a compromised output could tear down the control window or a sibling — and `destroy` skips that sibling's own `onCloseRequested`, which is exactly what makes `classifyDeparture` read the departure as a **crash**, three of which blocklist a working monitor for the session. The output now `preventDefault()`s and invokes `close_self`, which takes no label because Tauri supplies the calling window and can therefore only ever destroy itself (`lib.rs`). That works because of something this row used to claim the opposite of: **withholding `core:default` does not stop an output invoking app commands**. Tauri gates *plugin* commands, and app-defined ones only when the app ships its own permission manifest — this one ships none (bare `build.rs`, no `src-tauri/permissions/`), so `quit_app`, the keychain and the download commands are reachable from an output today. That is pre-existing, is what `close_self` rests on, and narrowing it is an open item in `docs/MULTI_MONITOR_PLAN.md` §6. Both halves are pinned by `acl_tests` in `lib.rs`, which puts a real `InvokeRequest` through the real capability files. It is defence in depth against a compromised output, **not** a restraint on the manager: Tauri checks window commands against the *calling* window, so putting the manager's grants here instead would deny every manager-initiated operation on an output |

...
| `src/services/datasetLoader.ts` | Loads a dataset onto the globe (HLS or image); manages info panel. `waitForDecodableFrame` is exported for the reason `pickDirectFile` is — it is where a wrong answer lives. It **nudges** the element with a muted `play()` rather than only listening for `canplay`, because WebKitGTK does not preroll a media pipeline until something plays it, and this wait runs *before* the `video.play()` that captures the first frame: on that engine each waited for the other, and a load with 88 s buffered and no error failed after 20 s naming the user's connection. Chrome, Firefox and Safari preroll on append, which is why one engine deadlocked and three did not. `readyState >= 3` short-circuits first so an element still warm from the previous dataset is not played, and a rejected `play()` costs the nudge alone — an autoplay policy strict enough to refuse a muted element belongs to an engine that needs no nudge. `playableStart` is the same rule reached from the other side: the first-frame capture used to rewind to a literal `0`, and the same WebKitGTK reading showed `buffered [6.0, 94.1]` on a 94.1 s asset — so a load that now succeeds would park a decodable element on a hole, with no frame and no error, which is what a frozen first frame looks like. It rewinds to the earliest instant that holds a frame, which is `0` on every engine buffering from the start and therefore perturbs nothing. An **empty** range is skipped before the comparisons, not after: a zero-length range at the origin otherwise wins `earliest` and returns the exact spot the function exists to avoid. It does not claim to explain the hole, only to decline to park in it, and it covers the load path alone — a native `loop` restart returns to 0 through the media element, where nothing here intercepts |

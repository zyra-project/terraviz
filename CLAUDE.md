...
>
> **Rungs 1-9 have landed** — the protocol, the equirect RTT pass, the
> output bundle and its layer stack, both capability files, the manager
> and state aggregator, the publish seam, the boot wiring, and now the
> Tools → Outputs panel. So the modules above are description, not
> intent, and the feature is reachable: on desktop an operator can add
> an output on a chosen monitor, toggle its camera tracking and sphere
> split, and remove it.
>
> Boot still does not call `manager.start()` — `outputUI` does, on the
> first add — so an install whose operator never opens the panel opens
> no IPC link and enumerates no monitor. That is the property to
> preserve: it is why the panel, not boot, owns the call.
>
> What rung 9 deliberately leaves for later rungs, so don't read their
> absence as oversight: persistence and rename (10), framebuffer
> resolution / decoder budget / debug overlay (11), the fullscreen and
> kiosk surfaces (12), health badges and noticing an output the operator
> closed by hand (13), calibration (14). **Nothing here has run on a
> second monitor** — rung 9's smoke checklist is the first that needs
> real hardware.
>

...
| `src/services/voiceWakeWord.ts` | Phase 3.5 wake-word ("Hey Orbit") — pure `WakeWordDetector` score→wake state machine (threshold / debounce / cooldown); the on-device openWakeWord (ONNX) audio pipeline lazy-loads on top in a later slice (`docs/ORBIT_VOICE_PLAN.md` §8 decision 5) |

...
> **Resolved upstream (2026-08-26).** tubeviz is now Apache-2.0: a `LICENSE` file, `license = "Apache-2.0"` plus `license-files` in `pyproject.toml`, a `NOTICE`, and an `SPDX-License-Identifier` header on every source file. Kept for the record only.

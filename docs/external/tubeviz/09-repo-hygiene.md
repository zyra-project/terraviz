...
> **Resolved upstream (2026-08-26).** The nested `src/src/` tree is deleted, no `.pyc` files are tracked, and the C++ source now has a single canonical home at `src/tubeviz/native_src/` with the root `native/` removed. `tests/test_repository_layout.py` asserts all of it, so it cannot silently regress. Still open: `.gitignore` has no `__pycache__` entry and still lists two entries twice. Kept for the record only.

...
There is also no automatic gradient legend on `main`. Generate one with
`scripts/make_legend.py` from the skill (`cool`, 0–10, `h/km²/month`)
and attach it as the dataset's `legend_ref` via Publish → Media. The
`color_scale` sidecar drives the hover readout either way.

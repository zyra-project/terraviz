...
| `functions/api/v1/_lib/tour-stop-order.ts` | Shot sequencing for generated event tours (`docs/TOUR_DIRECTION_PLAN.md` D1) — pure `orderTourStops`, maximal marginal relevance over the matched candidates: the strongest pairing opens, then each stop trades a little match score against how much it looks like what came before (facet + keyword Jaccard, bbox IoU; adjacency weighted over the whole selection). Stops four datasets the matcher scored together from running back to back. Deterministic, ties on id, no DB or fetch; an antimeridian-wrapping bbox scores no overlap rather than a wrong one. Not applied to the blog companion tour, whose stops are a curator's explicit picks |

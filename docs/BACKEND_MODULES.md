...
| `cli/lib/video-sitemap.ts` | Pure generic Google Video Sitemap → normalized video parser — the non-YouTube media-suggestion source. Same zero-dependency tolerant XML scan as `rss.ts`; sanitizes dirty feeds (strips stray `content_loc` quotes, collapses doubled slashes), filters junk tags (bare years, title echoes) and generic categories, surfaces each video's `contentHost` for the media-proxy host allowlist, detects/expands `<sitemapindex>` documents, and builds the canonical embedding text for semantic story matching |
...
| `functions/api/v1/publish/media/video-sources.ts` | GET (privileged list of registered non-YouTube Video Sitemaps for the Feeds console's Media tab) + POST (add a source `{ label, url, attribution?, enabled? }`; validated http(s) url, duplicate-url 409, audit `video_source.add`). Indexing happens on the next refresh |
| `functions/api/v1/publish/media/video-sources/[id].ts` | POST (patch a source — label/url/attribution/**enable-disable**) + DELETE (remove; `video_index` rows cascade) — privileged, audit-logged |
| `functions/api/v1/publish/media/video-sources/refresh.ts` | POST /api/v1/publish/media/video-sources/refresh — privileged on-demand indexer: runs the shared `refreshVideoSource` engine over every enabled source (fetch → parse → embed changed → upsert → prune), shared per-run embed budget, per-source run bookkeeping; 502 only on a total outage, else 200 with the per-source summary |
| `functions/api/v1/media/video-proxy.ts` | GET/HEAD /api/v1/media/video-proxy?url= — public CORS-adding, Range-forwarding stream proxy for direct video files, so the VR/AR tour path (which needs `Access-Control-Allow-Origin` for the `THREE.VideoTexture` upload — agency CDNs send none) can play a picked event video. NOT an open proxy: the URL's host must be on the `allowlistedContentHosts` set derived from enabled video sources; non-http / unknown-host / missing-DB refused before any upstream fetch |
| `functions/api/v1/publish/media/video-suggest.ts` | GET /api/v1/publish/media/video-suggest?q= — privileged: embeds the story text and cosine-ranks the node's indexed sitemap videos (`video_index`), returning `{ videos: [{ title, pageUrl, contentUrl, thumbnailUrl, attribution, score }] }`. Topic-only matching; degrades to `{ videos: [] }` (never errors) when AI is unconfigured or the index is empty |
...
| `functions/api/v1/publish/events/rematch.ts` | POST /api/v1/publish/events/rematch — privileged re-scoring of existing events against the current matcher. The matcher otherwise runs only on ingest and on a curator edit to date/geometry, so an event its feed has dropped keeps whatever the old formula produced (or, after migration `0044`, no score at all). Bounded per request and resumable by a keyset cursor on event id — not by re-querying the still-unscored set, which never terminates for an event that scores nothing. One event's failure is counted, not fatal |
...
| `functions/api/v1/_lib/tour-stop-order.ts` | Shot sequencing for generated event tours (`docs/TOUR_DIRECTION_PLAN.md` D1) — pure `orderTourStops`, maximal marginal relevance over the matched candidates: the strongest pairing opens, then each stop trades a little match score against how much it looks like what came before (facet + keyword Jaccard, bbox IoU; adjacency weighted over the whole selection). Deterministic, ties on id, no DB or fetch; an antimeridian-wrapping bbox scores no overlap rather than a wrong one. Not applied to the blog companion tour, whose stops are a curator's explicit picks |
...
| `functions/api/v1/_lib/events-matcher.ts` | Topical + temporal + geo scoring for current events — pure signals (curated topic-expanded term overlap, liveness-overlap, bbox-IoU) where topical relevance drives ranking and real-time/liveness boosts; `runMatcherForEvent` proposing `event_dataset_links` (`docs/CURRENT_EVENTS_PLAN.md` §4). Topical relevance is an IDF-weighted cosine (`buildIdf` over the candidate set, calibrated by `LEXICAL_REFERENCE`): it replaced a raw `min(1, 0.5 + 0.2 * overlap)` count that saturated at three shared terms, so ~25% of the catalogue tied at exactly 1.0 against a typical event and a long abstract outranked a precise title |
...
| `functions/api/v1/_lib/video-sources-store.ts` | `video_sources` data access — the node-configurable registry of non-YouTube Video Sitemaps for the media-suggestion engine (label, url, attribution, enabled, per-source run bookkeeping). The media counterpart of `feed-connectors-store`; read by the video-source refresh job, CRUD'd by the portal Feeds console |
| `functions/api/v1/_lib/video-source-refresh.ts` | Shared video-source refresh engine — fetch a registered sitemap (expanding a `<sitemapindex>` one level), parse + normalize via `cli/lib/video-sitemap`, (re)embed only entries whose text/model-version changed (budgeted; content-only + unembedded when AI is absent or the budget is spent), upsert into `video_index`, and prune entries that fell out. Never throws; the cron route records the per-source summary |
| `functions/api/v1/_lib/video-index-store.ts` | `video_index` data access — the materialized, embedded sitemap videos. Upserts each entry with its 768-dim BGE embedding stored as a Float32 BLOB (pack/unpack + in-Worker cosine helpers), prunes entries that fell out of the sitemap, derives the media-proxy / native-`<video>` host allowlist (`allowlistedContentHosts`) from enabled sources, and cosine-ranks the pool against a story embedding (`queryVideosBySimilarity`) |

...
| `CATALOG_R2_BUCKET` | Bucket name used to build the path-style S3 URL (`<endpoint>/<bucket>/<key>`) that presigned PUTs sign against — the `CATALOG_R2` binding does not supply it. Defaults to `terraviz-assets` to match `wrangler.toml`. Set it explicitly on any deploy whose bucket has a different name, in the Pages env **and** the GitHub Actions secret of the same name: left unset, the fallback addresses a bucket outside the API token's scope and R2 rejects every upload with a bare `401 Unauthorized`. | Phase 1b |

...
migrations/catalog/
  ...
  ...
migrations/catalog-schema.sql    -- post-migration snapshot, regenerated by
                                 -- `npm run db:dump-schema`. Lives outside
                                 -- migrations/catalog/ so Wrangler's migration
                                 -- runner does not pick it up as a migration.
```
...
- **Schema diff.** A second workflow dumps the resulting schema
  ...
  lives one directory above `migrations/catalog/` so Wrangler's
  migration runner — which globs every `.sql` under the migrations
  directory — does not pick it up as a migration. Schema changes
  require updating the snapshot in the same PR; the diff is the
  review artefact.

...
`vimeo:<id>` and resolved by the manifest endpoint (see
...
```
migrations/catalog/
  0001_init.sql                  -- node_identity + datasets (full Phase-1a columns)
  0002_decoration.sql            -- dataset_tags, dataset_categories, dataset_keywords,
                                 --   dataset_developers, dataset_related
  0003_renditions.sql            -- dataset_renditions + idx_renditions_dataset
  0004_tours.sql                 -- tours, tour_dataset_refs
  0005_publishers_audit.sql      -- publishers, audit_events, idx_audit_subject
  ...
schema/catalog-schema.sql        -- post-migration snapshot, regenerated by
                                 -- `npm run db:dump-schema`. Outside
                                 -- migrations/ entirely, so neither binding's
                                 -- migration runner can queue it.
```

Each migration is forward-only. Numeric prefix is monotonic and
gap-free; `wrangler d1 migrations apply CATALOG_DB` is the runner.
The full Phase-1a `datasets` schema lands in `0001_init.sql`,
including columns populated only by Phase 1b (content_digest,
media intrinsics, embedding_version) and Phase 6 (review-queue
lifecycle); all such columns are nullable so `schema_version`
stays at 1 across the milestone.

Federation (`federation_peers`, `federated_datasets`, …),
per-dataset grants, write-side collaboration, the review queue,
hard-delete stubs, and the docent featured-list arrive in
phase-targeted migrations starting at `0010_*`. Reserving the
0006-0009 range lets Phase 1b add columns or backfill helpers
without disturbing the Phase 4+ numbering.
...
- **Schema diff.** A second workflow dumps the resulting schema
  and diffs against a checked-in `schema/catalog-schema.sql`
  snapshot (regenerated by `npm run db:dump-schema`). The snapshot
  lives outside `migrations/` entirely. It used to sit one level
  above `migrations/catalog/` — which is `migrations/`, FEEDBACK_DB's
  own migrations directory, so Wrangler queued it against that
  binding. Schema changes require updating the snapshot in the same
  PR; the diff is the review artefact.
...
1. `wrangler d1 migrations apply CATALOG_DB` against production D1.

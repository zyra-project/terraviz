...
- Alpha error, mean by band — chroma key `0.504` in the 0.25–0.6 band; luminance
  over white `0.585` at a≥0.9; **data-as-luma `0.0027`** at a≥0.9.
...
- **Set colour range explicitly.** No `-color_range`/`-colorspace`/`-color_trc`
  flag exists anywhere in the repo today, so ffmpeg emits limited range (16–235)
  with unspecified VUI. A mismatch shifts every value by 16/255 ≈ 0.063 — larger
  than the smoke palette's entire `transparent_range` (12/256 ≈ 0.047). Measured:
  mismatched range leaked visible alpha on **99.99%** of empty pixels vs **0.10%**

...
| **3pf/D** — Portal multi-file uploader | Tabbed picker in `asset-uploader.ts`, parallel-bounded XHR queue, aggregate progress, frame-ordering controls (lexicographic by default; "Manual order" textarea fallback). A small "Display naming preview" panel renders what the consumer will see for frame 0 / frame N (computed from slug + start_time + period, or slug + index for non-time-series rows) so the publisher can sanity-check before submit. **No frame-rate picker** — output is always 30 fps. | Bulk of the UX work. New locale keys under `publisher.assetUploader.frames.*`. |

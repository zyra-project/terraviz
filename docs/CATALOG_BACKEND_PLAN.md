...
| Auth (admin) | **Cloudflare Access** | Already wired for `/api/feedback-admin`. Reused for the publisher portal initially. Also gates the CLI's service-token flow from Phase 1a onward. |
...
| Docent search index | **Vectorize** | Phase 1b. Managed vector DB on the same Cloudflare account, holds 768-dim embeddings of every dataset for the docent's `search_datasets` LLM tool. Federated datasets indexed by the local node at sync time. See "Docent integration". |
| Embedding model | **Workers AI** | Phase 1b. `@cf/baai/bge-base-en-v1.5` invoked from a Queue consumer to embed dataset text on publish / update / sync. Shares the same Workers AI binding as Orbit chat; budget is shared across both workloads. |
| Telemetry | **Analytics Engine** | Existing `terraviz_events`. New event types for catalog publish / federation sync / publisher actions / backend operational metrics land in the same stream behind the Tier A/B gate (see "Backend telemetry"). |
...
A Phase-1 importer reads `public/assets/sos-dataset-list.json` plus
`public/assets/sos_dataset_metadata.json`, runs the same merge that
`dataService.ts` does today, and inserts every record as a
`datasets` row owned by the deploying node with `visibility='public'`.
The dataset-list snapshot is fetched once from
`https://s3.dualstack.us-east-1.amazonaws.com/metadata.sosexplorer.gov/dataset.json`
and checked in; `npm run refresh:sos-snapshot` re-pulls it on
demand. Keeping the list local keeps `db:seed` deterministic and
CI-friendly. Vimeo links are preserved verbatim in `data_ref` as
`vimeo:<id>` and resolved by the manifest endpoint (see "Asset &
video pipeline"), so cutover doesn't require uploading anything to
Stream on day one.
...
| **Workers AI** | ~10k neurons/day | Shared budget across Orbit chat *and* (Phase 1b onward) the docent embedding pipeline. A 600-dataset catalog re-embed costs roughly 600 inferences against `bge-base-en-v1.5`; well under the daily quota. Steady-state churn (publish / update on a typical small org's cadence) is negligible. The cost surprise to watch is *bulk* re-embeds — model upgrades, large federation syncs from a high-churn peer — which should be enqueued and rate-limited rather than flooded synchronously. |
| **Cloudflare Access** | First 50 seats free, then $3/seat/mo | Fine for staff publishing through Phase 3. Phase 6 community publishers should arrive via OIDC instead, not as Access seats. Service tokens for the CLI are a separate Access-side concept and don't count against the seat cap. |
| **Analytics Engine** | 10M datapoints/mo write | Already in use for telemetry. New catalog / publisher / backend-operational events add to the same bucket — verify there is room (see "Backend telemetry" for the new event schema). |
| **Vectorize** | 30M queried dims/mo, 5M stored | **Required from Phase 1b** (was previously listed as optional). Comfortable for ~600 datasets with 768-dim embeddings (≈460k stored dims, well under the 5M cap). A federation deploy mirroring 5 peers' worth of catalogs at the same scale stays under 3M stored dims. The query budget is the bigger concern: every docent search-tool call costs 768 queried dims; 30M / 768 ≈ 39k searches per month, or ≈1.3k / day, which is enough for a small/medium deploy but worth measuring once usage is real. |
| **Cron Triggers** | 3 per Worker on free, more on paid | Phase 4 federation can run on cron alone (one trigger iterating peers). Beyond ~20 peers a per-peer cron stops fitting (see "Failure modes the protocol has to survive" in `CATALOG_FEDERATION_PROTOCOL.md`). |
...
| **3pd — Asset uploader + preview pipeline** | Reusable uploader component. **Images:** R2 presigned PUT, immediate `data_ref` write, digest verification. **Videos:** R2 presigned PUT of the source MP4 to `uploads/{dataset_id}/{upload_id}/source.mp4` (per-upload prefix so a re-upload to a transcoding row doesn't overwrite the bytes the prior workflow may still be reading), then a `repository_dispatch` to the GitHub Actions workflow that runs ffmpeg against the existing 4K / 1080p / 720p 2:1 spherical ladder (proven in [`cli/migrate-r2-hls.ts`](../cli/migrate-r2-hls.ts)), writes the HLS bundle to a versioned `videos/{dataset_id}/{upload_id}/` prefix in R2, and POSTs `/api/v1/publish/datasets/{id}/transcode-complete` on the publisher API. The route constructs `data_ref` server-side as `r2:videos/{id}/{upload_id}/master.m3u8` and clears `transcoding`. The portal polls status via a "Transcoding…" badge until the row is playable. "Preview" button mints a token via `POST .../preview`; the modal surfaces the backend's anonymous-read URL (`/api/v1/datasets/{id}/preview/{token}`), which returns the dataset's metadata as JSON (`{ dataset: row }`). Useful as a primitive for a reviewer who can fetch metadata; the SPA-side `?preview=…` consumer that opens the globe with full playback context is a Phase 3pe deliverable. | Heaviest single piece. Two upload paths, an external async pipeline, a status surface, and the preview hook. The Stream → R2-only shift means we own the transcoder (GHA + ffmpeg) — same code path the Phase 3 CLI migration already uses. |
...
| Risk | Mitigation |
|---|---|
| Stream costs scale with catalog size in ways Vimeo's static link did not. | Phase 1b backfill estimates total minutes before commit; budget review gates the cutover. Public deployment can keep `vimeo:` refs as long as needed. See "Free-tier viability." |
| Vectorize index is corrupted, deleted, or unavailable. | The catalog row in D1 is the source of truth; the vector is derived. The embed pipeline can be replayed end-to-end by enqueueing one job per dataset (`UPDATE datasets SET embedding_version = NULL` then run the cron). Recovery time is bounded by Workers AI throughput, not by Vectorize availability. The `embedding_version` column makes "what's already embedded vs. what's not" a single-query answer. |
| Workers AI quota exhaustion (mostly an issue on free-tier deploys with active Orbit chat AND active docent search). | The embedding budget is small and steady-state; the failure mode is a bulk re-embed (model upgrade or initial federation sync from a high-churn peer) flooding the daily neuron quota. Mitigated by enqueueing embed jobs to a Queue with a per-minute concurrency cap — bulk work spreads over hours rather than seconds, and the docent's runtime search budget is preserved. Operators with low-quota concerns can also configure the Queue to throttle harder during business hours. |
| Embedding pipeline backlog grows unboundedly under heavy publish/update churn. | Queue depth surfaces as a Grafana panel + an alert at "more than N pending for more than M minutes" (see "Backend telemetry"). A stuck job retries with exponential backoff; permanent failures emit an `embed_failed` audit event so the publisher portal can surface "this dataset is not searchable yet" cues. The dataset itself is still browsable and loadable — only docent recommendations are degraded. |
| Phase 1b scope is materially larger than the original Phase 1 was. | The internal 1a / 1b split exists specifically to give the team an early-checkpoint demo at the end of 1a (catalog backbone + minimal CLI), so a delay or surprise in 1b's Stream / asset / docent-search work never blocks shipping the more conservative milestone. The 1a exit criterion is independently demoable. |
